...
function ProgressRing({ value, max, label, unit, color, size = 100 }) {
 ...
 <span className="text-[10px] text-gray-400 dark:text-gray-500 font-medium">{unit}</span>
 </div>
 <span className="text-xs font-semibold text-gray-600 dark:text-gray-400 mt-1">{label}</span>
 </div>
 );
}
...
export default function Tracker() {
 ...
 <div>
 <h1 className="text-3xl font-display font-bold text-gray-900 dark:text-white">Calorie Tracker</h1>
 <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Track your daily nutrition intake</p>
 </div>
 </div>

 {/* Date Picker */}
 <div className="flex items-center justify-center gap-4 mb-8">
 <button onClick={() => changeDate(-1)} className="w-10 h-10 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-sm flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
 <svg className="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
 </svg>
 </button>
 <div className="text-center px-6 py-3 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm min-w-[260px]">
 <div className="text-base font-bold text-gray-900 dark:text-white">{formatDate(date)}</div>
 ...
 {/* Daily Totals */}
 <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-soft border border-gray-100 dark:border-gray-800 p-6 mb-8">
 <h2 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-5">Daily Summary</h2>
 ...
 <ProgressRing {...item} />
 <div className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">{Math.round(item.max - item.value)} left</div>
 </div>
 ))}
 </div>
 </div>
 ...
 <div className="w-8 h-8 border-4 border-gray-200 dark:border-gray-700 border-t-primary rounded-full animate-spin mx-auto mb-3" />
 Loading tracker...
 </div>
 ) : (
 ...
 return (
 <div key={key} className="bg-white dark:bg-gray-900 rounded-2xl shadow-soft border border-gray-100 dark:border-gray-800 overflow-hidden">
 {/* Section Header */}
 ...
 {meal.recipe_image ? (
 <img src={meal.recipe_image} alt="" className="w-12 h-12 rounded-xl object-cover border border-gray-100 dark:border-gray-800" />
 ) : (
 <div className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center text-lg">🍽️</div>
 )}
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2">
 <p className="font-semibold text-gray-900 dark:text-white text-sm truncate">{meal.meal_name || meal.recipe_title || 'Meal'}</p>
 ...
 {/* Add Meal Panel */}
 {isAdding && (
 <div className="px-6 py-5 bg-gray-50/80 border-t border-gray-100 dark:border-gray-800">
 {/* Recipe search */}
 <div className="mb-4">
 <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2 block">Search Recipe</label>
 <input
 type="text"
 placeholder="Search recipes to add..."
 value={searchQuery}
 onChange={e => searchRecipes(e.target.value)}
 className="w-full px-4 py-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10"
 />
 {searchLoading && <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Searching...</p>}
 ...
 <p className="text-xs text-gray-400 dark:text-gray-500">Click to add</p>
 </div>
 ...
 <span className="text-xs font-semibold text-gray-400 dark:text-gray-500">OR ADD MANUALLY</span>
 <div className="flex-1 border-t border-gray-200 dark:border-gray-700" />
 </div>
 ...
 className="px-3 py-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-center focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 placeholder-gray-300 dark:placeholder-gray-600"
 />
 ))}
 </div>
 <button
 ...
 <p className="text-gray-400 dark:text-gray-500 text-sm">No {label.toLowerCase()} logged</p>
 <button
