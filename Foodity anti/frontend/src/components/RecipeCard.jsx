  ...
  return (
    <div className="group block">
      <div className="card-white flex flex-col h-full">
        ...
        {/* Image container */}
        <Link to={`/recipe/${recipe.id}`} className="relative overflow-hidden aspect-[4/5] bg-gray-100 shrink-0 block">
          {!imgLoaded && (
          ...
          <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            ...
            <div className="absolute bottom-3 right-3 flex items-center gap-2">
              <button
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-md transition-all hover:scale-110 shadow-lg
                  ${liked ? 'bg-primary text-white' : 'bg-white/90 text-gray-700 hover:text-primary'}`}
              >
              ...
              <button
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-md transition-all hover:scale-110 shadow-lg
                  ${showMealPicker ? 'bg-green-500 text-white' : 'bg-white/90 text-gray-700 hover:text-green-600'}`}
              >
              ...
              <button
                onClick={handleShare}
                title="Share"
                className="w-10 h-10 rounded-full flex items-center justify-center bg-white/90 backdrop-blur-md text-gray-700 transition-all hover:scale-110 hover:text-blue-500 shadow-lg"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* Meal Picker Popover */}
            {showMealPicker && (
              <div className="absolute bottom-16 right-3 bg-white rounded-xl shadow-2xl border border-gray-100 py-1.5 z-20 animate-scale-in min-w-[140px]">
                {['breakfast', 'lunch', 'snacks', 'dinner'].map(m => (
                  <button
                    key={m}
                    onClick={(e) => handleAddToTracker(e, m)}
                    className="w-full px-4 py-2 text-left text-sm font-bold text-gray-700 hover:bg-orange-50 hover:text-primary capitalize transition-colors"
                  >
                    {m}
                  </button>
                ))}
          ...
          <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-100">
            <Link to={`/profile/${recipe.author?.id}`} className="flex items-center gap-2 group/author">
              {recipe.author?.avatar_url ? (
                <img src={recipe.author.avatar_url} alt="" className="w-6 h-6 rounded-full border border-gray-200 group-hover/author:border-primary transition-colors" />
              ) : (
                <div className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] group-hover/author:bg-primary/10 transition-colors">👤</div>
              )}
              <span className="text-xs font-medium text-gray-700 group-hover/author:text-primary transition-colors">
